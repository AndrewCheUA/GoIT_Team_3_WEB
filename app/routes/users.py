...
async def partial_update_user_profile(body: PartialProfileUpdate, user_id: int, db: AsyncSession = Depends(get_db),
                                      current_user: User = Depends(auth_service.get_current_user)) -> UserPublic:
    """
    The update_user_profile_partial function updates a user's profile.
        It takes in the following parameters:
            - body: PartialProfileUpdate (the partial update to be made)
            - user_id: int (the id of the user being updated)

    :param body: PartialProfileUpdate: Pass in the data to be updated
    :param user_id: int: Specify the user_id of the user to be updated
    :param db: AsyncSession: Get the database session
    :param current_user: User: Get the current user's id and use it to update their profile
    :return: A model of UserPublic object
    """
    user = await repository_users.get_user_by_id(current_user.id, db)
    ...
    if body.username and await repository_users.get_user_by_username(db = db, username = body.username):
        raise HTTPException(
            status_code = status.HTTP_400_BAD_REQUEST,
            detail = "That username is already taken. Please try another one."
        )

    return await repository_users.partial_update_user_profile(body, user_id, db)
